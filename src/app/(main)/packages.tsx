...
export default function PackagesScreen() {
  ...
  const chooseTeacher = (index: number) => setOptions((all) => all.map((option, i) => { if (i !== index) return option; const next = teachers[(teachers.findIndex((t) => t.id === option.teacherId) + 1) % Math.max(teachers.length, 1)]; const taught = next ? subjectsForTeacher(next.id) : []; return { teacherId: next?.id || "", subjectId: taught[0]?.id || "" }; }));
  const save = async () => {
